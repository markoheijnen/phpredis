                                 ...
                                 clusterReply **element, int *err)
{
    ...
    while(elements-- > 0) {
        ...
        switch(r->type) {
            case TYPE_ERR:
            case TYPE_LINE:
                if(redis_sock_gets(sock,buf,sizeof(buf),(void *)r->len)<0) {
                    *err = 1;
                    return;
                }
                break;
...
clusterReply *cluster_read_resp(redisCluster *c) {
    return cluster_read_sock_resp(c->cmd_sock,c->reply_type,c->reply_len);
}
                          ...
                          int key_len, clusterKeyVal **kv)
{
    ...
    if((ppdl = zend_hash_index_find_ptr(ht, (ulong)slot))==NULL) {
        ...
        ZVAL_PTR(&z_dl, dl);
        zend_hash_index_update(ht, (ulong)slot, &z_dl);
    } else {
        dl = (clusterDistList *)ppdl;
    }
                        ...
                        )
{
    ...
    // Serialize our value
    val_free = redis_serialize(c->flags, z_val, &val);
...
void cluster_multi_fini(clusterMultiCmd *mc) {
    ...
    smart_str_appendl(&(mc->cmd), mc->args.s->val, mc->args.s->len);
}
...
static void ht_free_slave(zval *data) {
    if(Z_PTR_P(data)) {
        cluster_free_node((redisClusterNode*)Z_PTR_P(data));
    }
}
...
cluster_init_seeds(redisCluster *cluster, HashTable *ht_seeds) {
    ...
    ZEND_HASH_FOREACH_VAL(ht_seeds, z_seed) {
...
PHP_REDIS_API int
cluster_map_keyspace(redisCluster *c) {
    RedisSock *seed;
...
PHP_REDIS_API void cluster_disconnect(redisCluster *c) {
    ...
    ZEND_HASH_FOREACH_PTR(c->nodes, node) {
        redis_sock_disconnect(node->sock);
                              ...
                              int direct)
{
    ...
    } else if (!cluster_dist_write(c, cmd, sz, 0)) {
    ...
    ZEND_HASH_FOREACH_PTR(c->nodes, seed_node) {
        ...
        if(seed_node->sock == redis_sock || seed_node->slave) {
            continue;
        }
                                  ...
                                  int cmd_len)
{
    ...
    do {
        ...
        if (resp == 1) {
           ...
           if (c->redir_type == REDIR_MOVED) cluster_update_slot(c);
        }
                              ...
                              void *ctx)
{
    ...
    if (CLUSTER_IS_ATOMIC(c)) {
        ...
        zval *z = NULL;
        if (redis_unserialize(c->flags, resp, c->reply_len, &z)) {
            efree(resp);
                             ...
                             void *ctx)
{
    ...
    while(1) {
        ...
        if(!z_tab || (z_type = zend_hash_index_find(Z_ARRVAL_P(z_tab), 0))
                                          == NULL)
        {
            break;
        }
                               ...
                               redisCluster *c, void *ctx)
{
    ...
    while(argc--) {
        ...
        if((z_flag = zend_hash_index_find(Z_ARRVAL_P(z_tab), 2)) == NULL || Z_STRLEN_P(z_flag) != 2)
        {
            zval_dtor(z_tab);
            efree(z_tab);
            zval_dtor(return_value);
                                   ...
                                   redisCluster *c, mbulk_cb cb, void *ctx)
{
    ...
    if(CLUSTER_IS_ATOMIC(c)) {
        ZVAL_COPY(return_value, &z_result);
        efree(&z_result);
    } else {
                                     ...
                                     redisCluster *c, int pull, mbulk_cb cb)
{
    ...
    // Call our callback
    if(cb(c->cmd_sock, &z_result, c->reply_len, NULL)==FAILURE) {
        zval_dtor(&z_result);
        efree(&z_result);
        return NULL;
    }
                                     ...
                                     redisCluster *c, void *ctx)
{
    ...
    zval_dtor(return_value);
    ZVAL_COPY(return_value, &c->multi_resp);
    efree(&c->multi_resp);
}
                              ...
                              void *ctx)
{
    ...
    if(mctx->last) {
        if(CLUSTER_IS_ATOMIC(c)) {
            ZVAL_BOOL(return_value, Z_TYPE_P(mctx->z_multi) == IS_TRUE ? 1 : 0);
        } else {
            add_next_index_bool(&c->multi_resp, Z_TYPE_P(mctx->z_multi) == IS_TRUE ? 1 : 0);
        }
        efree(mctx->z_multi);
    }
                    ...
                    long long count, void *ctx)
{
    ...
    while(count--) {
        ...
        if (line != NULL) {
            zval *z = NULL;
            if(redis_unserialize(redis_sock, line, line_len, &z)==1) {
    ...
    while(count--) {
        ...
        if (line != NULL) {
            if(idx++ % 2 == 0) {
                ...
                zval *z = NULL;
                if (redis_unserialize(redis_sock,key,key_len, &z)) {
                    convert_to_string(z);
