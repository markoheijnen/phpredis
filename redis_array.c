...
#define RA_CALL_FAILED(rv, cmd) \
    (((Z_TYPE_P(rv) == IS_TRUE || Z_TYPE_P(rv) == IS_FALSE) && Z_LVAL_P(rv) == 0) || \
 ...
 * redis_array_get
 */
PHP_REDIS_API int redis_array_get(zval *id, RedisArray **ra)
{
    ...
    *ra = (RedisArray *) Z_RES_P(socket)->ptr;
    ...
    if (!*ra || Z_RES_P(socket)->type != le_redis_array) {
        return -1;
    }
...
PHP_METHOD(RedisArray, __construct)
{
    ...
    if(z_opts) {
        ...
        if ((zpData = zend_hash_str_find(hOpts, "index", sizeof("index") - 1)) != NULL && (Z_TYPE_P(zpData) == IS_TRUE || Z_TYPE_P(zpData) == IS_FALSE)) {
            b_index = Z_LVAL_P(zpData);
        }

        /* extract autorehash option. */
        if ((zpData = zend_hash_str_find(hOpts, "autorehash", sizeof("autorehash") - 1)) != NULL && (Z_TYPE_P(zpData) == IS_TRUE || Z_TYPE_P(zpData) == IS_FALSE)) {
            b_autorehash = Z_LVAL_P(zpData);
        }

        /* pconnect */
        if ((zpData = zend_hash_str_find(hOpts, "pconnect", sizeof("pconnect") - 1)) != NULL && (Z_TYPE_P(zpData) == IS_TRUE || Z_TYPE_P(zpData) == IS_FALSE)) {
    ...
    switch(Z_TYPE_P(z0)) {
        ...
        case IS_ARRAY:
            ra = ra_make_array(Z_ARRVAL_P(z0), &z_fun, &z_dist, hPrev, b_index, b_pconnect, l_retry_interval, b_lazy_connect, d_connect_timeout);
            break;
...
PHP_METHOD(RedisArray, __call)
{
    ...
    if (redis_array_get(object, &ra) < 0) {
        RETURN_FALSE;
    }
...
#define HANDLE_MULTI_EXEC(cmd) do {\
    if (redis_array_get(getThis(), &ra) >= 0 && ra->z_multi_exec) {\
        int i, num_varargs;\
        zval *varargs = NULL;\
        zval z_arg_array;\
        if (zend_parse_method_parameters(ZEND_NUM_ARGS(), getThis(), "O*",\
...
PHP_METHOD(RedisArray, mget)
{
    ...
    ZEND_HASH_FOREACH_VAL(h_keys, data) {
        ...
        /* Find our node */
        redis_instances[i] = ra_find_node(ra, key_lookup, key_len, &pos[i]);
...
PHP_METHOD(RedisArray, mset)
{
    ...
    /* associate each key to a redis node */
    i = 0;
    ZEND_HASH_FOREACH_KEY_VAL(h_keys, idx, key, data) {
        ...
        redis_instances[i] = ra_find_node(ra, key->val, (int)key->len, &pos[i]);
        argc_each[pos[i]]++;    /* count number of keys per node */
        argv[i] = data;
        keys[i] = key->val;
    ...
    for(n = 0; n < ra->count; ++n) { /* for each node */
        ...
        if(ra->index) {
            ra_index_keys(z_argarray, redis_inst); /* use SADD to add keys to node index */
            ra_index_exec(redis_inst, NULL, 0); /* run EXEC */
        }
...
PHP_METHOD(RedisArray, del)
{
    ...
    if (redis_array_get(getThis(), &ra) < 0) {
        RETURN_FALSE;
    }
    ...
    ZEND_HASH_FOREACH_VAL(h_keys, data) {
        ...
        redis_instances[i] = ra_find_node(ra, Z_STRVAL_P(data), Z_STRLEN_P(data), &pos[i]);
        argc_each[pos[i]]++;    /* count number of keys per node */
        argv[i] = data;
        ++i;
    } ZEND_HASH_FOREACH_END();
    ...
    for(n = 0; n < ra->count; ++n) { /* for each node */
        ...
        if(ra->index) {
            ra_index_del(z_argarray, redis_inst); /* use SREM to remove keys from node index */
            ra_index_exec(redis_inst, &z_tmp, 0); /* run EXEC */
...
PHP_METHOD(RedisArray, exec)
{
    ...
    if (redis_array_get(object, &ra) < 0 || !ra->z_multi_exec) {
        RETURN_FALSE;
    }
